 ...
 */
 ...
 class BankController extends BaseController{
     ...
     public function DataList(){
         ...
         $Title=I('post.Title');
         if($Title){
            $where['BankName']=array('like','%'.$Title.'%');
         }
     ...
     public function Save(){
         ...
         //验证规则
         $rules=array(
             array('BankName','require','银行名称必须填写'),
             array('Logurl','require','银行Logo图必须上传'),
         );
     ...
     public function Del(){
         ...
         //如果该材料被使用则无法删除
         $count=M(self::T_ITEMS)->where(array('BankID'=>array('in',$idArr),'IsDel'=>0))->count();
         if($count>0){
             $this->ajaxReturn(0,"该银行正在使用中，无法删除");
         }
