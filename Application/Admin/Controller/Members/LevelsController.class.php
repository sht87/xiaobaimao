 ...
 */
 ...
 class LevelsController extends BaseController{
     ...
     public function Save(){
         ...
         //验证规则
         $rules=array(
             array('Name','require','代理名称必须填写'),
             array('Price','require','购买价格必须填写'),
         );
     ...
     public function Del(){
         ...
         //如果被使用则无法删除
         $exit=M(self::T_MEMINFO)->where(array('Mtype'=>array('in',$idArr),'IsDel'=>0))->find();
         if($exit){
             $this->ajaxReturn(0,"该代理正在使用中，无法删除");
         }
