...
class XBUpload extends Controller
{
    ...
    public function uploadimage(){
        ...
        $PicSize = get_basic_info('PicSize')*1024;
        $PicExt = explode(',',get_basic_info('PicExt'));

        $upload = new \Think\Upload(); // 实例化上传类
        
        $upload->maxSize   = $PicSize;
        $upload->exts      = $PicExt;
        ...
        if(!$info) {// 上传错误提示错误信息
            ...
            foreach ($info as $v){
                ...
                $thumbnail = get_basic_info('IsThumbnail'); //是否生成缩略图，后期可将此开关放在后台设置
                if($thumbnail){
                    //获取原图保存地址
    ...
    public function uploadfile(){
        ...
        $PicSize = get_basic_info('FileSize')*1024;
        $PicExt = explode(',',get_basic_info('FileExt'));
        //$bucket = 'file'; 
    ...
    public function QiniuUpload($filetype,$file=''){
        ...
        foreach($info as $val){
            if($val['Name']=='AppKey') {$secretKey=$val['ParaValue'];}
            if($val['Name']=='SecretKey') {$accessKey=$val['ParaValue'];}
            if(empty($domin)) {
