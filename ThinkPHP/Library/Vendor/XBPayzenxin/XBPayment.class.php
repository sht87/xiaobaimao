...
use Vendor\XBPayzenxin\Gateways\Chanpay\Payer;
...
//        if(!isset($compact['type']) && !in_array($compact['type'],array(0,1))) return $this->ajaxReturn(0,'请刷新后提交');
//        if(!isset($compact['uid']) && !in_array($compact['uid'],array(0,1))) return $this->ajaxReturn(0,'请刷新后提交');
        if($compact['type']){
            $Type=$compact['type'];
            unset($compact['type']);
        }
        ...
        if(!$OrderInfo) return $this->ajaxReturn('订单已失效');
        ...
        //调用支付
        $Chanpay=XBPaymentFactory::$payload['Chanpay'];


        HuryUtils::recordNotifyStep('test', 'test', $Chanpay, true);
...
//        );
        if($Type==1){
            $Chanpay['return_url']='/daizenxin/index?uid='.$UserID;
        }
        $payer = new Payer($Chanpay);
...
//        );
        ...
        if(!$OrderInfo = $order->find($OrderNo)) return $this->ajaxReturn(0, '订单有误，请重新下单');
        if($OrderInfo['Type']==1){
            $Chanpay['return_url']='/daizenxin/index?uid='.$OrderInfo['UserID'];
        }
