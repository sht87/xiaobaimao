...
class ImgCode {
    ...
    public function check($code) {
        ...
        // 验证码不能为空
        $cache=new XBCache();
        $secode = $cache->GetCache('code');
        if(empty($code) || empty($secode['verify_code'])) {
            return false;
        }
        // 缓存 过期
        if(NOW_TIME - $secode['verify_time'] > 60) {
            XBCache::Remove('code', null);
            return false;
        }

        if($code== $secode['verify_code']) {
            $this->reset && XBCache::Remove('code', null);
            return true;
        }
    ...
    public function entry() {
        ...
        // 保存验证码
        $secode     =   array();
        $secode['verify_code'] = $code; // 把校验码保存到缓存中
        $secode['verify_time'] = NOW_TIME;  // 验证码创建时间

        XBCache::Insert('code',$secode);
        // 输出图像
